Module SFMpqModule
    ...
    Private MPQInit As Boolean = False
    ...
    Public Sub MPQ_WriteFile(MPQ As String, File As String, ByteData() As Byte, Optional Flags As MPQ_FileFlags = MPQ_FileFlags.Compress, Optional Locale As MPQ_Locale = MPQ_Locale.Neutral)
        Dim hMPQ As IntPtr
    ...
    Public Sub MPQ_PutFile(MPQ As String, File As String, FilePath As String, Optional Flags As MPQ_FileFlags = MPQ_FileFlags.Compress, Optional Locale As MPQ_Locale = MPQ_Locale.Neutral)
        Dim hMPQ As IntPtr
        hMPQ = MPQWrite_OpenMPQ(MPQ)
    ...
    Public Function MPQRead_ReadFile(MPQ As IntPtr, File As String, Optional Locale As MPQ_Locale = MPQ_Locale.English) As Byte()
        ...
        ByteData = Array.CreateInstance(GetType(Byte), Math.Max(Size, 0))
        Do Until Size <= 0
            If SFileReadFile(hFile, ByteData(Read), Size, Read, 0&) = False Then Return Nothing
            Size -= Read
        Loop
    ...
    Public Function MPQRead_ReadWavFile(MPQ As IntPtr, File As String, Optional Locale As MPQ_Locale = MPQ_Locale.English) As Byte()
        Dim ByteData() As Byte, hFile As IntPtr, Size As Integer, Read As Integer
        SFileSetLocale(Locale)
        If SFileOpenFileEx(MPQ, File, 0, hFile) = False Then Return Nothing
        ByteData = Array.CreateInstance(GetType(Byte), 8)
        If SFileReadFile(hFile, ByteData(Read), 8, Read, 0&) = False Then Return Nothing
        Size = BitConverter.ToUInt32(ByteData, 4)
        Array.Resize(Of Byte)(ByteData, Size + 8)
        Do Until Size <= 0
            If SFileReadFile(hFile, ByteData(Read), Size, Read, 0&) = False Then Return Nothing
            Size -= Read
        Loop
    ...
    Public Sub MPQWrite_WriteFile(MPQ As IntPtr, File As String, ByteData() As Byte, Optional Flags As MPQ_FileFlags = MPQ_FileFlags.Compress, Optional Locale As MPQ_Locale = MPQ_Locale.Neutral)
    ...
    Public Sub MPQWrite_PutFile(MPQ As IntPtr, File As String, FilePath As String, Optional Flags As MPQ_FileFlags = MPQ_FileFlags.Compress, Optional Locale As MPQ_Locale = MPQ_Locale.Neutral)
        MpqAddFileToArchiveEx(MPQ, FilePath, File, Flags, &H8, 0)
        If Not Locale = 0 Then MpqSetFileLocale(MPQ, File, 0, Locale)
    End Sub

    Public Sub MPQWrite_WriteWavFile(MPQ As IntPtr, File As String, ByteData() As Byte, Optional CompressLevel As MPQ_WaveLevel = MPQ_WaveLevel.High, Optional Flags As MPQ_FileFlags = MPQ_FileFlags.Compress, Optional Locale As MPQ_Locale = MPQ_Locale.Neutral)
        If ByteData Is Nothing Then
    ...
    Public Sub MPQWrite_PutWavFile(MPQ As IntPtr, File As String, FilePath As String, Optional CompressLevel As MPQ_WaveLevel = MPQ_WaveLevel.High, Optional Flags As MPQ_FileFlags = MPQ_FileFlags.Compress, Optional Locale As MPQ_Locale = MPQ_Locale.Neutral)
        MpqAddWaveToArchive(MPQ, FilePath, File, Flags, CompressLevel)
        If Not Locale = 0 Then MpqSetFileLocale(MPQ, File, 0, Locale)
    End Sub

    Public Sub MPQWrite_Compact(MPQ As IntPtr)
        Call MpqCompactArchive(MPQ)
    End Sub
End Module
